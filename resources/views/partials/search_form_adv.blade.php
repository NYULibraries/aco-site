  ...
  <form class="advanced" method="get" action="{{ url('search') }}" role="search">
    <div class="fieldset group1">
          ...
          <select class="scope-select" aria-label="Boolean Operator">
            @foreach ($form['scopes'] as $item)
              <option value="{{ $item['value'] }}"
                @if ($item['value'] === $form['scope']) selected @endif>
                  {{ $item['label'] }}
              </option>
            @endforeach
          </select>
      ...
      <div class="input-hold">
        <input value="{{ $form['q'] }}" class="q1" aria-label="Search"  name="q" type="text" placeholder="search  /  ابحث" title="Enter the terms you wish to search for.">
      </div>
